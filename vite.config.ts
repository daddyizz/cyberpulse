...
function cyberPlayerReliabilityPatch(): Plugin {
  const replaceOnce = (code: string, search: string, replacement: string, label: string) => {
    ...
    if (code.indexOf(search, first + search.length) !== -1) {
      throw new Error(`[CyberPulse patch] Ambiguous block: ${label}`);
    }
  ...
  return {
    ...
    transform(source, id) {
      ...
      // Full lyrics modal must control the real media source too.
      code = code.replace(
        `          onTogglePlayPause={() => setIsPlaying(!isPlaying)}`,
        `          onTogglePlayPause={togglePlayPause}`
      );
