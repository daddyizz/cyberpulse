  ...
  return {
    ...
    transform(source, id) {
      ...
      code = replaceOnce(
        ...
        `  // Never substitute a different song when media resolution fails.\n  const effectiveYtId = currentTrack.youtubeVideoId;\n\n  // If Spotify cannot provide a full-length stream, one verified YouTube player becomes\n  // the audio+video transport. Audio/Video mode then only changes presentation, not timing.\n  const isYouTubeActive = Boolean(effectiveYtId);`,
