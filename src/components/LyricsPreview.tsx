...
export function LyricsPreview({
  ...
  const rootIsLight = typeof document !== 'undefined' && document.documentElement.classList.contains('theme-pure_light');
  const isLight = theme ? theme === 'pure_light' : rootIsLight;
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4 no-scrollbar">
        {isLoading ? (
          ...
          lyrics.map((line, idx) => {
            ...
            return (
              <div key={idx} ref={isActive ? activeLineRef : null} onClick={() => onSeek(line.timeMs / 1000)} className={`p-2.5 rounded-xl transition-all cursor-pointer select-none text-center ${
                isActive
                  ? isLight ? 'text-slate-900 font-black text-lg bg-slate-100 scale-105 shadow-sm' : 'text-[#CCFF00] font-black text-lg bg-[#CCFF00]/10 scale-105 shadow-sm'
                  : isPassed
                  ? isLight ? 'text-slate-400 text-sm font-medium hover:text-slate-900' : 'text-[#8E8E93]/60 text-sm font-medium hover:text-white'
                  : isLight ? 'text-slate-600 text-sm font-medium hover:text-slate-900' : 'text-[#8E8E93] text-sm font-medium hover:text-white'
              }`}>{line.text}</div>
            );
          })
        ) : (
          <div className="py-12 flex flex-col items-center justify-center space-y-3 text-center px-4">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isLight ? 'bg-slate-100 text-slate-400' : 'bg-[#141416] text-[#8E8E93]'}`}><AlertCircle className="w-6 h-6" /></div>
            <div>
  ...
  if (!isModal) {
    return <div className={`w-full max-w-[280px] h-[280px] rounded-3xl overflow-hidden border p-4 shadow-xl ${isLight ? 'bg-white border-slate-200 text-slate-900' : 'bg-[#0C0C0E] border-[#242428] text-white'}`}>{content}</div>;
  }
    ...
    <div className={`sona-lyrics-modal absolute inset-0 z-50 backdrop-blur-2xl flex flex-col pt-12 sm:pt-14 px-5 pb-6 animate-in fade-in duration-200 ${isLight ? 'bg-white/98 text-slate-900' : 'bg-[#000000]/98 text-white'}`}>
      <div className={`flex items-center justify-between pb-4 border-b ${isLight ? 'border-slate-200' : 'border-[#242428]'}`}>
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isLight ? 'bg-slate-100 text-slate-900' : 'bg-[#141416] text-[#CCFF00]'}`}><Music className="w-5 h-5" /></div>
          <div>
        ...
        {onClose && <button onClick={onClose} className={`p-2 rounded-full transition-colors cursor-pointer ${isLight ? 'text-slate-500 hover:text-slate-900 hover:bg-slate-100' : 'text-[#8E8E93] hover:text-white hover:bg-[#141416]'}`} title="Close lyrics"><X className="w-6 h-6" /></button>}
      </div>
      ...
      <div className={`pt-4 border-t flex items-center justify-between ${isLight ? 'border-slate-200' : 'border-[#242428]'}`}>
        ...
        <button onClick={onTogglePlayPause} className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-transform active:scale-95 cursor-pointer ${isLight ? 'bg-slate-900 text-white shadow-slate-900/20' : 'bg-[#CCFF00] text-black shadow-[#CCFF00]/30'}`}>{isPlaying ? <Pause className="w-5 h-5 fill-current" /> : <Play className="w-5 h-5 fill-current ml-0.5" />}</button>
