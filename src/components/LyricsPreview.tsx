...
export function LyricsPreview({
    ...
    <div className={`flex flex-col ${isModal ? 'h-full' : 'max-h-[280px]'} w-full`}>
      ...
      <div
        className={`flex items-center justify-between pb-3 border-b text-xs ${
          isLight ? 'border-slate-200 text-slate-500' : 'border-[#242428] text-[#8E8E93]'
        }`}
      >
        <div className="flex items-center gap-1.5">
          <span
            className={`w-2 h-2 rounded-full ${
              isLight ? 'bg-slate-900' : 'bg-[#CCFF00] animate-pulse'
            }`}
          />
          <span
            className={`font-mono text-[11px] font-semibold ${
              isLight ? 'text-slate-900' : 'text-[#CCFF00]'
            }`}
          >
            Synced Database LRC
          </span>
        ...
        <span
          className={`text-[10px] px-2 py-0.5 rounded-full border ${
            isLight
              ...
              : 'bg-[#141416] border-[#242428] text-[#8E8E93]'
          }`}
        >
          Tap line to jump
        </span>
          ...
          <div className="py-12 flex flex-col items-center justify-center space-y-3">
            <div
              className={`w-6 h-6 border-2 border-t-transparent rounded-full animate-spin ${
                isLight ? 'border-slate-900' : 'border-[#CCFF00]'
              }`}
            />
            <p className={`text-xs font-mono ${isLight ? 'text-slate-500' : 'text-[#8E8E93]'}`}>
              Fetching verified synchronized lyrics...
            </p>
          </div>
          ...
          <div className="py-12 flex flex-col items-center justify-center space-y-3 text-center px-4">
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                isLight ? 'bg-slate-100 text-slate-400' : 'bg-[#141416] text-[#8E8E93]'
              }`}
            >
            ...
            <div>
              ...
              <p className={`text-xs mt-1 ${isLight ? 'text-slate-500' : 'text-[#8E8E93]'}`}>
                No synchronized lyrics database entry found for &ldquo;{track.title}&rdquo;
              </p>
            </div>
          </div>
        )}
  ...
  if (!isModal) {
    return (
      <div
        className={`w-full max-w-[280px] h-[280px] rounded-3xl overflow-hidden border p-4 shadow-xl ${
          isLight
            ? 'bg-white border-slate-200 text-slate-900'
            : 'bg-[#0C0C0E] border-[#242428] text-white'
        }`}
      >
        {content}
      </div>
  ...
  return (
    <div
      className={`absolute inset-0 z-50 backdrop-blur-2xl flex flex-col pt-12 sm:pt-14 px-5 pb-6 animate-in fade-in duration-200 ${
        isLight
          ? 'bg-white/98 text-slate-900'
          : 'bg-[#000000]/98 text-white'
      }`}
    >
      {/* Modal Top Navigation - Cleared safely below phone status bar */}
      <div
        className={`flex items-center justify-between pb-4 border-b ${
          isLight ? 'border-slate-200' : 'border-[#242428]'
        }`}
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center ${
              isLight ? 'bg-slate-100 text-slate-900' : 'bg-[#141416] text-[#CCFF00]'
            }`}
          >
          ...
          <div>
            <h3
              className={`text-base font-black uppercase truncate max-w-[200px] sm:max-w-xs ${
          ...
          <button
            ...
            className={`p-2 rounded-full transition-colors cursor-pointer ${
              isLight
                ...
                : 'text-[#8E8E93] hover:text-white hover:bg-[#141416]'
            }`}
            title="Close lyrics"
          >
            <X className="w-6 h-6" />
      ...
      >
        <div className={`text-xs font-mono ${isLight ? 'text-slate-500' : 'text-[#8E8E93]'}`}>
          {formatSecs(currentSeconds)}
        </div>
        <button
        ...
        <div className={`text-xs font-mono ${isLight ? 'text-slate-500' : 'text-[#8E8E93]'}`}>
          {formatSecs(track.durationSeconds || 214)}
        </div>
      </div>
    </div>
  );
